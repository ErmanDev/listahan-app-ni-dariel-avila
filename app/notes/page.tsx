...
export default function NotesApp() {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      // Skip resize handling if it's likely caused by mobile keyboard
      const heightDiff = Math.abs(windowHeight - window.innerHeight);
      const isKeyboardResize = heightDiff > 150;
      
      if (!isKeyboardResize) {
        if (window.innerWidth < 768) {
          // Only set sidebar visibility on initial load or explicit toggle
          if (!selectedNote) {
